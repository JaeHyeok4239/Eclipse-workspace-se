...
public class NotRuntimeExceptionCatchMain3 {

	public static void main(String[] args) {
		try {
			System.out.println("stmt1");
			/*
			 예외발생예상코드(UnknownHostException, IOException)
			  1.예외발생시 IOException객체생성 IOException객체가 던져짐
			...
			/*
			 예외발생예상코드(ClassNotFoundException)
			  1.예외발생시 ClassNotFoundException객체생성 ClassNotFoundException객체가 던져짐
			...
			/*
			 예외발생예상코드(ArithmeticException[RuntimeException])
				1.예외발생시 ArithmeticException객체생성 ArithmeticException객체가 던져짐
			...
			e.printStackTrace();
			JOptionPane.showMessageDialog(null, "에러메세지:"+e.getMessage());
			System.exit(0);
		}
